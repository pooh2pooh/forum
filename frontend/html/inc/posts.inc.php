<?php
	...
	foreach ($data['posts'] as $row)
	{
		...
		$login = $this->getLoginByID($row['author']);
		$username = $this->getUserNameByID($row['author']);
		$timestamp = new DateTime($row['date'], new DateTimeZone('Europe/Moscow'));
		$avatar = $this->getUserAvatarByID($row['author']);
		$avatar = !empty($avatar) ? 'uploads/avatars/thumbs/' . $avatar : 'https://via.placeholder.com/150';
		...
		$usernameTemplate = "
			<h6>
				<img src='$avatar' class='d-inline d-md-none' width='32px;'>
				<a class='d-none d-md-block h3 fw-bold align-middle text-dark' href='profile.php?user=$login'>$username</a>
				<a class='d-inline d-md-none h6 fw-bold align-middle text-dark' href='profile.php?user=$login'>$username</a>
	...
	?>
			...
			<div class="d-none d-md-block col-md-2">
				<?php if(!isMyPost($login)) { ?>
					<img src="<?=$avatar?>" class="img-fluid sticky-top" style="top: 10px;">
				<?php } ?>
			</div>
			<div class="col-12 col-md-10 mb-1">
				<?php if(!isMyPost($login)) {
					echo $usernameTemplate;
				} ?>
